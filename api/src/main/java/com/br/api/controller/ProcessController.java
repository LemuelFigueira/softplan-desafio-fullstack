...
public class ProcessController {
        ...
        @GetMapping("/search")
        @SecurityRequirement(name = SecurityType.BEARER)
        public ProcessPageableResponseDTO findByAnyNameOrProfileOrEmail(
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "5") int size,
                        @RequestParam(defaultValue = "") String query) {
                return processService.findByAnyTitleOrSubtitleOrDescription(query, page, size);
        }
        ...
        @PutMapping("/{id}")
        public void update(@PathVariable @NonNull Long id,
                        @RequestBody @Validated ProcessRequestDTO processRequestDTO) {
